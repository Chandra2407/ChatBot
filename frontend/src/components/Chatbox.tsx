...
const Chatbox: React.FC<ChatboxProps> = ({ message = { id: '', text: '', type: 'user', avatar: '' }, messages = [], index }) => {
    return (
        <div className={`message ${message.type} flex items-center gap-2 max-w-[calc(100%-2rem)] ${message.type === 'user' ? 'ml-auto flex-row-reverse bg-gray-800' : 'mr-auto bg-cyan-900'} mb-2 px-2 py-4 rounded-lg w-fit`}>
            {
